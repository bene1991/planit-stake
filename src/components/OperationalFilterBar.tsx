...
interface OperationalFilterBarProps {
  ...
  onFilterChange: (filters: OperationalFilters) => void;
}
...
export function OperationalFilterBar({ methods, leagues, filters, onFilterChange }: OperationalFilterBarProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  const handlePeriodChange = (period: OperationalFilters['period']) => {
    const now = new Date();
  ...
  return (
    <Card className="p-4 shadow-card">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* Período */}
        <Select value={filters.period} onValueChange={(v) => handlePeriodChange(v as OperationalFilters['period'])}>
            ...
            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
              {leagues.map((league) => (
                <div key={league} className="flex items-center gap-1.5">
                  ...
                  />
                  <Label htmlFor={`op-league-${league}`} className="text-xs cursor-pointer">
                    {league}
                  </Label>
                </div>
              ))}
